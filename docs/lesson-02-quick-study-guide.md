...
Condensed review for exams. See the [full Lesson 2 guide](lesson-02-transport-application.md) for detail. Practice interactively with the **[Lesson 2 Quiz](lesson-02-quiz.md)**.

---

## 1. Why transport exists
...
**Memory aid:** IP gets packets *near* the host; transport gets bytes to the *right app*.
...
```
IP: best effort, datagram, host-to-host
Transport: segment, process-to-process

UDP: fast, no guarantees
TCP: SYN-SYN/ACK-ACK, FIN teardown, reliable stream

Fast retransmit: 3 dup ACKs
Flow: rwnd (receiver) | Congestion: cwnd (network)
Send ≤ min(cwnd, rwnd)

Slow start: exponential until ssthresh
Avoidance: +1 MSS/RTT, sawtooth
Dup ACK → cwnd/2 | Timeout → reset hard

CUBIC: W(t)=C(t-K)^3+Wmax, β=0.2, time not RTT, TCP-friendly on short RTT
Throughput ∝ 1/(RTT·√p)
```
